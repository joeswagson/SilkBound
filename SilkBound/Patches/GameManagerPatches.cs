...
namespace SilkBound.Patches
{
    ...
    public class GameManagerPatches
    {
        ...
        {
            if (NetworkUtils.IsConnected && NetworkUtils.LocalConnection != null && TransactionManager.Fetch<SaveDataTransfer>(saveSlot) == null)
            { 
                SaveDataTransfer transfer = new SaveDataTransfer(
                ...
                for (int i = 0; i < chunks.Count; i++)
                {
                    ...
                    NetworkUtils.LocalConnection!.Send(new TransferSaveDataPacket(chunk, i, chunks.Count, transferId));
                }
